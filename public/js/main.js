class PasteEditor {
  ...
  initializeErrorElements() {
    try {
      ...
      if (
        ...
        !this.errorPopupOverlay ||
        !this.errorPopupButton
      ) {
  ...
  initializeTurnstile() {
    try {
      ...
      if (!window.TURNSTILE_SITE_KEY || window.TURNSTILE_SITE_KEY === "") {
        console.warn(
          "Turnstile site key not configured properly. Using development fallback."
        );
      ...
      const waitForTurnstile = () => {
        if (window.turnstile) {
          // Render the widget
          this.turnstileWidget = window.turnstile.render(
            "#turnstile-container",
            {
  ...
  validateTabState() {
    ...
    const domTabIds = Array.from(domTabs).map((tab) =>
      parseInt(tab.getAttribute("data-tab"))
    );
      ...
      if (isMultiTab || isPrivate) {
        ...
        if (isMultiTab && isPrivate) {
          warningText.textContent =
            "Disabled due to MULTI_TAB and PRIVATE PASTE";
        } else if (isMultiTab) {
  ...
  updateWordCount() {
    ...
    const asciiCount = (text.match(/[ -~]/gu) || []).length;
  ...
  rebuildTabs() {
    ...
    Array.from(this.tabs.entries()).forEach(([id, data]) => {
      ...
      if (this.tabs.size > 1) {
        ...
        deleteBtn.innerHTML =
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18"></path><path d="M6 6l12 12"></path></svg>';
  ...
  handleTabDrag(e) {
    ...
    const tabs = Array.from(
      this.tabsContainer.querySelectorAll(".tab:not(#new-tab):not(.dragging)")
    );
    ...
    if (
      this.dragState.currentDropTarget &&
  ...
  handleTabDragEnd() {
    ...
    const draggedTabId = parseInt(
      this.dragState.draggedTab.getAttribute("data-tab")
    );
  ...
  async submitPaste() {
    try {
      ...
      // Validate and prepare tabs
      const nonEmptyTabs = this.prepareTabsForSubmission();
      if (!nonEmptyTabs) return;

      // Validate tab size and security verification
      if (!this.validateTabSizes(nonEmptyTabs)) return;
      if (!this.validateSecurityVerification()) return;
      ...
      // Prepare paste data
      const pasteData = this.preparePasteData(nonEmptyTabs);
      if (!pasteData) {
      ...
      await this.submitPasteData(pasteData);
    } catch (error) {
  ...
  /**
   * Prepare tabs for submission by filtering empty tabs
   * @returns {Array|null} Array of non-empty tabs or null if validation fails
   */
  prepareTabsForSubmission() {
  ...
  /**
   * Validate tab size limits
   * @param {Array} tabs - Tabs to validate
   * @returns {boolean} True if all tabs are within size limits
   */
  validateTabSizes(tabs) {
    const MAX_TAB_SIZE = 2 * 1024 * 1024; // 2MB

    for (const tab of tabs) {
      const contentSize = new Blob([tab.content]).size;
      if (contentSize > MAX_TAB_SIZE) {
  ...
  /**
   * Validate security verification (Turnstile)
   * @returns {boolean} True if security verification is valid
   */
  validateSecurityVerification() {
    if (
      !this.turnstileToken &&
  ...
  /**
   * Prepare paste data object with all required fields
   * @param {Array} tabs - Non-empty tabs
   * @returns {Object|null} Paste data object or null if validation fails
   */
  preparePasteData(tabs) {
    // Base paste data
    const pasteData = {
      tabs,
      token: this.turnstileToken || "development_token",
    };
  ...
  /**
   * Submit paste data to server and handle response
   * @param {Object} pasteData - Paste data to submit
   */
  async submitPasteData(pasteData) {
  ...
  /**
   * Handle errors during paste submission
  ...
  getOptions() {
    return {
      ...
      password: this.isPrivateCheckbox?.checked
      ...
      expiry: this.enableExpiryCheckbox?.checked
        ...
        : undefined,
      burnCount: this.enableBurnCheckbox?.checked
        ? parseInt(document.getElementById("burn-count").value)
        : undefined,
    };
  }
